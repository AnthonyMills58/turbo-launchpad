...
const rateLimiting_1 = require("./core/rateLimiting");
...
async function acquireGlobalLock() {
    const lockClient = await db_1.default.connect();
    try {
        const { rows } = await lockClient.query('SELECT pg_try_advisory_lock($1, $2) AS locked', [config_1.LOCK_NS, config_1.LOCK_ID]);
        if (!rows[0]?.locked) {
            lockClient.release();
            return null;
        }
        return {
            release: async () => {
                await lockClient.query('SELECT pg_advisory_unlock($1, $2)', [config_1.LOCK_NS, config_1.LOCK_ID]);
                lockClient.release();
            }
        };
    }
 ...
 */
async function checkChainHealth(chainId, provider) {
    try {
        console.log(`Checking health for chain ${chainId}...`);
        // Try to get the latest block number with a timeout
        const latestBlock = await Promise.race([
            (0, rateLimiting_1.withRateLimit)(() => provider.getBlockNumber(), 2, chainId),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Health check timeout')), config_1.HEALTH_CHECK_TIMEOUT))
        ]);
        // Try to get block details to ensure the chain is actually responding
        const block = await Promise.race([
            (0, rateLimiting_1.withRateLimit)(() => provider.getBlock(latestBlock), 2, chainId),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Block fetch timeout')), config_1.HEALTH_CHECK_TIMEOUT / 2))
        ]);
        if (!block) {
...
async function main() {
    ...
    try {
        ...
        const { rows: chains } = await db_1.default.query('SELECT DISTINCT chain_id FROM public.tokens ORDER BY chain_id');
        console.log(`📊 Found ${chains.length} chains:`, chains.map(c => c.chain_id));
        // Health check before processing
        if (!config_1.SKIP_HEALTH_CHECK) {
            console.log(`\n🔍 Checking chain health before processing...`);
            const healthyChains = [];
            for (const { chain_id } of chains) {
                const provider = (0, providers_1.providerFor)(chain_id);
 ...
 */
async function processChain(chainId) {
    console.log(`🔗 Setting up provider for chain ${chainId}...`);
    const provider = (0, providers_1.providerFor)(chainId);
    ...
    const { rows: tokens } = await db_1.default.query(`
 ...
 */
async function processToken(token, provider, chainId) {
    console.log(`🔍 Getting current block for token ${token.id}...`);
    const currentBlock = await (0, rateLimiting_1.withRateLimit)(() => provider.getBlockNumber(), 2, chainId);
    ...
    const chunkSize = (0, config_1.getChunkSize)(chainId);
    ...
    for (let from = startBlock; from <= endBlock; from += chunkSize + 1) {
        ...
        const toBlockInfo = await (0, rateLimiting_1.withRateLimit)(() => provider.getBlock(to), 2, chainId);
        ...
        try {
            ...
            if (dexPool) {
                ...
                if (from >= dexDeploymentBlock) {
                    ...
                    try {
                        ...
                        const dexUpdateResult = await db_1.default.query(`
            ...
            `, [token.id, chainId, dexPool.pair_address, to, dexPool.token0, dexPool.token1, dexPool.quote_token, dexPool.token_decimals, dexPool.weth_decimals, dexPool.quote_decimals, dexPool.deployment_block]);
                        ...
                        const dexVerifyResult = await db_1.default.query(`
        ...
        `, [token.id, to, `Token ${token.id}`, `TKN${token.id}`, 1000000, 100, 'uniswap', token.creator_wallet || '', token.chain_id]);
                ...
                const verifyResult = await db_1.default.query(`
 ...
 */
async function processTransferChunk(token, dexPool, provider, chainId, fromBlock, toBlock) {
    // Get transfer logs for token
    const transferLogs = await (0, rateLimiting_1.withRateLimit)(() => provider.getLogs({
        address: token.contract_address,
        topics: [TRANSFER_TOPIC],
        fromBlock,
        toBlock
    }), config_1.MAX_RETRY_ATTEMPTS, chainId);
    ...
    if (isGraduationBlockRange) {
        ...
        for (const [txHash, logs] of logsByTx) {
            const tx = await (0, rateLimiting_1.withRateLimit)(() => provider.getTransaction(txHash), config_1.MAX_RETRY_ATTEMPTS, chainId);
            if (!tx) {
            ...
            if (isGraduation) {
                ...
                const block = await (0, rateLimiting_1.withRateLimit)(() => provider.getBlock(firstLog.blockNumber), config_1.MAX_RETRY_ATTEMPTS, chainId);
 ...
 */
async function processDexLogsForChain(token, dexPool, provider, chainId, fromBlock, toBlock) {
    ...
    // Get timestamp for the last block in range for better debugging
    const toBlockInfo = await (0, rateLimiting_1.withRateLimit)(() => provider.getBlock(toBlock), 2, chainId);
    const toBlockTimestamp = toBlockInfo ? new Date(Number(toBlockInfo.timestamp) * 1000).toISOString() : 'unknown';
    ...
    const pairAddress = ethers_1.ethers.getAddress(dexPool.pair_address);
    ...
    const swapLogs = await (0, rateLimiting_1.withRateLimit)(() => provider.getLogs({
        address: pairAddress,
        topics: [SWAP_TOPIC],
 ...
 */
async function processTransferLog(token, dexPool, log, provider, chainId) {
    try {
        // Parse transfer log
        const fromAddress = ethers_1.ethers.getAddress('0x' + log.topics[1].slice(26));
        const toAddress = ethers_1.ethers.getAddress('0x' + log.topics[2].slice(26));
        const amount = BigInt(log.data);
        // Get transaction details with more conservative retry
        let tx = null;
        try {
            tx = await (0, rateLimiting_1.withRateLimit)(() => provider.getTransaction(log.transactionHash), config_1.MAX_RETRY_ATTEMPTS, chainId);
        }
        catch (error) {
        ...
        // Get block timestamp
        const block = await (0, rateLimiting_1.withRateLimit)(() => provider.getBlock(log.blockNumber), config_1.MAX_RETRY_ATTEMPTS, chainId);
        const blockTime = new Date(Number(block.timestamp) * 1000);
 ...
 */
async function detectGraduation(token, log, tx, provider, chainId) {
    // Check if this transaction contains graduation by looking for Graduated event
    try {
        const receipt = await (0, rateLimiting_1.withRateLimit)(() => provider.getTransactionReceipt(tx.hash), config_1.MAX_RETRY_ATTEMPTS, chainId);
        if (receipt) {
            // Check for Graduated event
            const graduatedEvent = receipt.logs.find(log => log.topics[0] === GRADUATED_TOPIC);
            if (graduatedEvent) {
            ...
            if (transferLogs.length >= 3) {
                ...
                const mintToUser = transferLogs.find(log => {
                    const fromAddr = ethers_1.ethers.getAddress('0x' + log.topics[1].slice(26));
                    const toAddr = ethers_1.ethers.getAddress('0x' + log.topics[2].slice(26));
 ...
 */
async function createGraduationRecords(token, log, tx, blockTime, provider, chainId, dexPool) {
    // Get all transfer logs for this transaction to find the correct amounts
    const allLogs = await (0, rateLimiting_1.withRateLimit)(() => provider.getLogs({
        address: token.contract_address,
        topics: [TRANSFER_TOPIC],
    ...
    const userBuyLog = txLogs.find(l => {
        const fromAddr = ethers_1.ethers.getAddress('0x' + l.topics[1].slice(26));
        const toAddr = ethers_1.ethers.getAddress('0x' + l.topics[2].slice(26));
        return fromAddr === '0x0000000000000000000000000000000000000000' &&
            toAddr !== token.contract_address;
    });
    ...
    const userToAddress = ethers_1.ethers.getAddress('0x' + userBuyLog.topics[2].slice(26));
    const lpToAddress = lpTransferLog ? ethers_1.ethers.getAddress('0x' + lpTransferLog.topics[2].slice(26)) : '0x0000000000000000000000000000000000000000';
    ...
    try {
        ...
        if (receipt) {
            ...
            const addLiquidityInterface = new ethers_1.ethers.Interface([
 ...
 */
function determineTransferType(token, tx, fromAddress, toAddress) {
 ...
 */
async function processRegularTransfer(token, dexPool, log, tx, fromAddress, toAddress, amount, blockTime, chainId, provider) {
    ...
    if (transferType === 'BUY' || transferType === 'BUY&LOCK') {
        ...
        try {
            const receipt = await (0, rateLimiting_1.withRateLimit)(() => provider.getTransactionReceipt(log.transactionHash), config_1.MAX_RETRY_ATTEMPTS, chainId);
            if (receipt && receipt.blockNumber) {
                const blockBeforeTx = receipt.blockNumber - 1;
                const turboTokenInterface = new ethers_1.ethers.Interface([
                    'function getSellPrice(uint256 tokenAmount) view returns (uint256)'
                ]);
                const sellPriceWei = await (0, rateLimiting_1.withRateLimit)(() => provider.call({
 ...
 */
async function processDexLog(token, dexPool, log, provider, chainId) {
    try {
        // Get transaction details
        const tx = await (0, rateLimiting_1.withRateLimit)(() => provider.getTransaction(log.transactionHash), config_1.MAX_RETRY_ATTEMPTS, chainId);
        if (!tx) {
        ...
        const swapInterface = new ethers_1.ethers.Interface([
