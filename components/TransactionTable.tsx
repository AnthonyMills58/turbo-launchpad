...
export default function TransactionTable({ tokenId, tokenSymbol, creatorWallet }: TransactionTableProps) {
  ...
  const [availableTransactionTypes, setAvailableTransactionTypes] = useState<Array<{side: string, count: string}>>([])
  ...
  // Simple TraderDisplay component showing only address
  ...
  const fetchTransactions = useCallback(async (page: number = 1) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCurrentPage(page)
        setAvailableTransactionTypes(data.transactionTypes || [])
      }
    } catch (error) {
  ...
  const formatETHValue = (ethAmount: number): string | JSX.Element => {
    if (ethAmount < 0.001) {
